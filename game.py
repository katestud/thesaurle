...
def play_the_game(path, num_turns):
    ...
    print(
        f"You can get from {start_word} to {target_word} in {dist_to_target} steps")
    ...
    while not game_won and turns_taken < factor * num_turns:
        ...
        if not (guess in guess_options):
            ...
            if guess == target_word:
                ...
                dist_to_target = len(
                    fetch_shortest_path(guess, target_word)) - 1
                print(
                    f"Guesses so far: {taken_guesses}. Try to get to {target_word}. You are at least {dist_to_target} steps away")
